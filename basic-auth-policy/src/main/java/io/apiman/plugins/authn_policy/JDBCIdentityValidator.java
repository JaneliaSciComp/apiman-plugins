...
import io.apiman.plugins.authn_policy.config.JDBCIdentitySource;
import io.apiman.plugins.authn_policy.config.JDBCType;
        ...
        IAsyncResultHandler<IJdbcResultSet> roleHandler = new IAsyncResultHandler<IJdbcResultSet>() {
            ...
            public void handle(IAsyncResult<IJdbcResultSet> result) {
                if (result.isError()) {
                    ...
                    context.setAttribute(AUTHENTICATED_USER_ROLES, extractedRoles);
                    closeQuietly(connection);
