...
import io.apiman.plugins.authn_policy.config.BasicAuthenticationConfig;
...
public class BasicAuthenticationPolicy extends AbstractMappedPolicy<BasicAuthenticationConfig> {
    ...
    protected void sendAuthFailure(IPolicyContext context, IPolicyChain<?> chain, BasicAuthenticationConfig config, int reason) {
        ...
        PolicyFailure failure = pff.createFailure(PolicyFailureType.Authentication, reason, Messages.format("BasicAuthenticationPolicy.AuthenticationFailed")); //$NON-NLS-1$
