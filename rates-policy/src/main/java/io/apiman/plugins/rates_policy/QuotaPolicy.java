...
public class QuotaPolicy extends RateLimitingPolicy {
    ...
    protected PolicyFailure limitExceededFailure(IPolicyFailureFactoryComponent failureFactory) {
        PolicyFailure failure = failureFactory.createFailure(PolicyFailureType.Other, PolicyFailureCodes.REQUEST_QUOTA_EXCEEDED, Messages.format("QuotaPolicy.QuotaExceeded")); //$NON-NLS-1$
